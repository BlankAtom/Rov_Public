...
double ReceiveManager::getRoundAngle(quint8 high, quint8 low)
{
    int sum = 0;
    int h = high << 8;
    int l = low;
...
double ReceiveManager::getAngle(quint8 high, quint8 low)
{
    ...
    double result = ((double) sum ) /100.0 - 90;
    return result;
}
...
{
    int a = this->command[9];
    int b = this->command[10];
    int c = this->command[11];
    double result =( a << 16) + (b << 8 ) + c;
...
{
    return getRoundAngle(this->command[12], this->command[13]);
}
